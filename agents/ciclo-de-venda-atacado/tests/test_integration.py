...
@pytest.mark.asyncio
async def test_consultar_bq_allowed_dataset(mcp_url, headers):
    from mcp.client.streamable_http import streamable_http_client
    from mcp.client.session import ClientSession
    ...
    async with streamable_http_client(mcp_url, headers=headers) as (r, w):
        async with ClientSession(r, w) as session:
            ...
            result = await session.call_tool(
                "consultar_bq",
                {"sql": "SELECT 1 FROM `soma-dl-refined-online.atacado_processed.info_venda` LIMIT 1"},
            )
    content_text = str(result.content)
    assert "dataset_not_allowed" not in content_text
