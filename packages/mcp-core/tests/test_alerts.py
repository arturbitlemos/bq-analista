...
async def test_detects_high_calls_per_hour(tmp_path: Path) -> None:
...
async def test_detects_huge_query(tmp_path: Path) -> None:
    log = AuditLog(db_path=tmp_path / "a.db")
    await log.record(
...
async def test_detects_error_rate(tmp_path: Path) -> None:
    log = AuditLog(db_path=tmp_path / "a.db")
    for i in range(20):
        await log.record(
            exec_email="e@x.com", tool="consultar_bq", sql="SELECT 1",
